...
class AuthController extends GetxController {
  ...
  PersistentTabController layoutViewCtr=PersistentTabController(initialIndex: 0);
  ...
  Future<String?> checkUserCredentials(String userName, String pwd) async {
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        // Assuming email field is present in the document
  ...
  ///  EMAIL 'signIn' + 'signUp'
  signIn(String _usrName, String _password) async {
    try {
      print('## Email signing In ...');

      String? _email = await checkUserCredentials(_usrName, _password);
  ...
  void signOutUser({bool shouldGoLogin = false,bool dbLogOut = false,}) async {
    try {
      ...
      if (shouldGoLogin) {
        await goLogin(email: cUser.userName);
      }
